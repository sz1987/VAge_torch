...
def read_obj(file):
    ...
    for i, line in enumerate(fp):
    	...
    	#Number of vertices
             ...
             XYZ[i-1]=[float(s) for s in line.split()]
             # XYZ[i-1]=map(float,line.split())
        #  elif i>2*n_vert+5:
        #      if not line.strip():
        #          k=1
        #      elif k==1:
        #          # print('line_{}, extend => {}'.format(i, line.split()))
        #          Polys.extend(line.split())
    print('len of last line => {}'.format(len(line.split())))
    Polys = [int(part) for part in line.split()]
    # print('Polys length => {}'.format(len(Polys)))
    print('there are {} point index in the obj file'.format(len(Polys)))
    # Polys=list(map(int,Polys))
    # print('after map')
    # print(Polys)
    npPolys=np.array(Polys)
    list_chunks = list(chunks(Polys,3))
    print(list_chunks[:3])
    print(list_chunks[-4:])
    # print('list_chuncks.len => {}'.format(len(list_chunks)))
